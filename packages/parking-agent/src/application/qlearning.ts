...
export class QTable {
  ...
  /** Select the best action with random tie-breaking. */
  bestAction(sKey: string): { action: Action; value: number; secondBest: number } {
    ...
    const ties = actions.filter(a => this.get(sKey, a) === bestV);
    const action = ties[Math.floor(Math.random() * ties.length)];
    ...
    return { action, value: bestV, secondBest: second };
  }
